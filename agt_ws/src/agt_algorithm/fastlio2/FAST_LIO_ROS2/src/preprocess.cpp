...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1)
{
  ...
  yaw_crop_enable = false;
  yaw_crop_min_deg = -180.0;
  yaw_crop_max_deg = 180.0;
}
...
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg)
{
  ...
  uint valid_num = 0;
  auto in_yaw_crop_sector = [this](float x, float y) -> bool {
    if (!yaw_crop_enable) {
      return true;
    }
    const double yaw_angle = atan2(y, x) * 57.29577951308232;
    return yaw_angle >= yaw_crop_min_deg && yaw_angle <= yaw_crop_max_deg;
  };
  ...
  if (feature_enabled)
  {
    for (uint i = 1; i < plsize; i++)
    {
      ...
      {
        if (!in_yaw_crop_sector(msg->points[i].x, msg->points[i].y))
        {
          continue;
        }
...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  for (uint i = 0; i < plsize; ++i)
  {
    ...
    if (yaw_crop_enable && (yaw_angle < yaw_crop_min_deg || yaw_angle > yaw_crop_max_deg))
    {
      continue;
    }
